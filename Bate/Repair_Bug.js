...
Ext.onReady(function() {
    ...
    const StuPersonNew = {
        ...
        listeners: {
            afterrender: function(me) {
                ...
                const zt = [['1', '居民身份证'], ['6', '香港特区护照/身份证明'], ['7', '澳门特区护照/身份证明'], ['8', '台湾居民来往大陆通行证'], ['9', '境外永久居住证'], ['A', '护照'], ['C', '港澳台居民居住证']];
                const frm = Ext.create('Ext.form.Panel', {
                    ...
                    items: [
                        ...
                        // 第一行
                        {
                            ...
                            name: 'spno',
                            width: 180,
                            renderer: (value) => SctCoz.tools.TransValue(value, 'MajorNoStore', 'spno', 'text'),
                        },
                        ...
                        { fieldLabel: '状态', name: 'changetype', width: 140 },
                        {
                            ...
                            readOnly: true,
                            name: 'enrolldate',
                            renderer: window.ShowFormatDate,
                        },
                        // 第二行
    ...
    const StuDelayNew = {
        ...
        listeners: {
            afterrender: function(me) {
                const tmSto = Ext.data.StoreManager.lookup('xqSto');
                const dlSto = Ext.create('Ext.data.Store', {
                ...
                const cnoSto = Ext.create('Ext.data.Store', {
                ...
                const ztAry = ['未提交', '已提交', '审核通过', '不通过'];
                const qryfrm = Ext.create('Edu.view.QueryForm', {
                    url: '/student/stuinfo',
                    labelWidth: 60,
                    region: 'north',
                    argcols: [
                ...
                qryfrm.load();
                const dlGd = Ext.create('Edu.view.ShowGrid', {
                    region: 'center',
                    width: '50%',
                    plugins: [
                    ...
                    columns: [
                        {
                            ...
                            header: '操作',
                            width: 70,
                            renderer: (v, m, r) => (r.data.zt === 0 ? ['提交', '删除'] : []),
                            items: [
                                {
                                    handler: function(grid, rowIndex) {
                                        const sto = grid.getStore();
                                        const rec = sto.getAt(rowIndex);
                                        if (rec.get('zt') === 0) {
                                            const ly = rec.get('reason');
                                            if (!ly || ly.trim().length < 4) {
                                                return Ext.Msg.alert('提示', '必须输入缓考理由');
                                            }
                                            rec.set('zt', 1);
                                            sto.sync();
                                            return;
                ...
                function queryStore() {
                    const args = qryfrm.getForm().getValues();
                ...
                function addXk() {
                    const rs = grid.getSelectionModel().getSelection();
                    if (rs.length < 1) {
                        Ext.Msg.alert('请选择学生');
                    }
                    const qm = qryfrm.getValues();
                    rs.forEach(function(m) {
                        const nr = new dlSto.model();
                ...
                // 隐藏多余的旧模块
                me.addListener('add', function(me, newPan) {
                    newPan.hide();
                });
            },
        },
    ...
    SctCoz.tools.menuChange(StuPersonNew);
    SctCoz.tools.menuChange(StuDelayNew);
});
