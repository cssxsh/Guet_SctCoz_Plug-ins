    ...
    afterrender: (me) => {
        ...
        const queryForm = Ext.create("SctCoz.Query.QueryForm", {
            argcols: [
                ...
                { fieldLabel: "节次", labelWidth: 32, width: 64, name: "startsequence" },
        ...
        const queryGrid = Ext.create("SctCoz.Query.QueryGrid", {
            ...
            columns: [
                ...
                { header: "备注", dataIndex: "comment", xtype: "actionrendercolumn", flex: 1, renderer: (value, metaData, record) => (record.get("comment") !== null ? ["查看"] : [""]), items: [{ handler: showComm }] }
            ],
            newTbar: [
                {
                    xtype: "button",
    ...
    afterrender: (me) => {
        ...
        const evalGrid = Ext.create("Ext.grid.Panel", {
            ...
            columns: [
                ...
                { header: "指标", dataIndex: "xh", width: 64, renderer: (value, metaData, record) => record.get("nr") },
            ...
            dockedItems: [
                {
                    ...
                    dock: "bottom",
                    layout: { pack: "center" },
                    items: [{ text: "保存", action: "save", formBind: true, handler: evalSave }, { text: "提交", action: "submit", formBind: true, handler: evalSave }]
                }
            ]
        });
...
const load = () => {
    ...
    content.addListener("add", (me, lastTab) => {
        lastTab.addListener("add", (me) => {
            const grids = me.query("grid,showgrid");
            grids.forEach((item) => {
