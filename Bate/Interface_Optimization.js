...
const CourseSetNewListeners = {
  afterrender: me => {
    ...
    const queryByStore = button => {
      ...
      params.startsequence = isNaN(params.startsequence)
        ? 1
      ...
      grid.printConfig.title =
        formSet.findField("term").getDisplayValue() + "课程设置";
    };
...
const StuScoreNewListeners = {
  afterrender: me => {
    ...
    const queryByStore = button => {
      ...
      store.getProxy().extraParams = params;
      grid.printConfig.title =
        formSet.findField("term").getDisplayValue() + "学生成绩";
      store.load();
    };
    const queryForm = Ext.create("SctCoz.Query.QueryForm", {
      argcols: [
...
const StuPlanNewListeners = {
  afterrender: me => {
    ...
    const queryByStore = button => {
      ...
      // 设置打印标题
      grid.printConfig.title =
        formSet.findField("term").getDisplayValue() +
        formSet.findField("plan").getDisplayValue();
    };
    const queryForm = Ext.create("SctCoz.Query.QueryForm", {
      argcols: [
        // xtype里已经封装好了store。
...
const StuEvalNewListeners = {
  afterrender: me => {
    ...
    const queryByStore = button => {
      ...
      // 设置打印标题
      grid.printConfig.title =
        formSet.findField("term").getDisplayValue() + "评教列表";
    };
    const queryForm = Ext.create("SctCoz.Query.QueryForm", {
      argcols: [
        // xtype里已经封装好了store。
    ...
    const loadCourseEvalNo = (record, grid) => {
      ...
      if (ctrl.eval_hide) {
        panel
          .down("button[action='save']")
          .setVisible(!record.get("chk") && ctrl.eval_hide);
        panel
      ...
      Ext.Ajax.request({
        ...
        success: response => {
          ...
          if (result.success) {
            ...
            const queryGrid = form
              .up("[xtype='panel']")
              .down("[xtype='query-grid']");
            const queryRecord = queryGrid
              .getStore()
              .findRecord("courseno", params.courseno);
    ...
    listeners: {
      afterrender: me => {
          ...
          listeners: {
            load: (me, data) => {
              ...
              SctCoz.tools.LoadData({
                path: "NewInfo.json",
                success: response => {
                  const data = Ext.isArray(response.data)
                    ? response.data
                    : [response.data];
        ...
        const editfrm = Ext.create("Ext.form.Panel", {
          ...
          region: "north",
          html:
            "<table width='100%'><tr><td><p>尊敬的用户：</p><p>　　您好，欢迎使用桂林电子科技大学教务管理系统。</p></td></tr></table>"
        });
