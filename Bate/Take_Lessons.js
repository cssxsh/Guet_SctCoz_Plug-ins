...
const Rreplace_StuSct = me => {
  ...
  const loadCourseNo = record => {
    ...
    if (ctrl.all) {
      ...
      const records = newStore.AllQueryStore.getGroups().find(
        item => item.name === courseid
      ).children;
          ...
          Ext.Ajax.request({
            ...
            success: response => {
              ...
              if (result.success) {
                ...
                "课程:" + params.courseno + "选择失败，选课人数已满!" ===
                  result.msg ||
                ctrl.overflow
              ) {
    ...
    groupHeaderTpl: [
      "<div>[{children:this.formatText}]共{children.length}个课号</div>",
      {
        formatText: function(records) {
          ...
          let text = TransValue(spno, "MajorNoStore", "spno", "text");
      ...
      const planGrid = Ext.create("SctCoz.Query.QueryGrid", {
        ...
        listeners: {
          select: function(me, record) {
            ...
            const grid = this.up("[xtype='query-panel']").down(
              "grid[region='south']"
            );
            const store = grid.getStore();
            store.getProxy().extraParams = params;
